...
export class PolylineTool implements EditorTool {
  id = 'polyline';
  label = 'Polyline';
  icon = 'pencil'; // or 'spline' if available

  private machine = new PathMachine();
  private static PREVIEW_ID = 'polyline_tool_preview';
  ...
  onExit(): void {
    this.machine.reset();
    PreviewManager.clearPreview(PolylineTool.PREVIEW_ID);
  }
  ...
  onPointerMove(evt: PointerInfo): void {
    ...
    if (this.machine.isDrawing() && rawPoint) {
      ...
      PreviewManager.showPath(PolylineTool.PREVIEW_ID, state.points, rawPoint, false);
    }
  }
